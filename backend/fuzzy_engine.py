...
def calculate_fuzzy(duration_val, count_val):
    # Definisi semesta pembicaraan (berdasarkan dokumen Tugas 2)
    ...
    # Membership Function (MF) sesuai grafik di dokumen
    ...
    # ... tambahkan rule lainnya sesuai tabel di dokumen
    ...
    duration_val= np.clip(duration_val, 0,12)
    count_val=np.clip(count_val,0,8)
    
    sim.input['duration'] = duration_val
    sim.input['count'] = count_val
    sim.compute()
    
    return sim.output['density']

if __name__ == "__main__":
    # Menerima input dari Bun
